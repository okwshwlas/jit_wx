...
// Node object
function Node(id, pid, name,selected ,url, title, target, icon, iconOpen, open) {
	//
...
dTree.prototype.add = function(id, pid, name,selected, url, title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name,selected, url, title, target, icon, iconOpen, open);
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		if(node.selected==true){
			str += '<input type="checkbox" name="id" checked="checked" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id+':'+node.name +'" class="cx"/>';
		}else{
			str += '<input type="checkbox" name="id" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id+':'+node.name +'" class="cx"/>';	
		}
